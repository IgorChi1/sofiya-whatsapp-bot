...
class CommandHandler {
    constructor(bot) {
        ...
        this.commands = {
            ...
            'создатьаренду': this.createRental.bind(this),
            'продлитьаренду': this.extendRental.bind(this),
    ...
    async checkAccess(messageInfo, command) {
        ...
        // Команды управления арендой только для владельца
        if (['удалитьаренду', 'создатьаренду', 'продлитьаренду'].includes(command)) {
    ...
    /**
     * Команда: создатьаренду - создание аренды (только владелец)
     * Формат: #создатьаренду <план>
     */
    async createRental(messageInfo, args) {
        try {
            const { chatId } = messageInfo;
            
            if (args.length === 0) {
                await this.bot.sendMessage(chatId, '❌ Укажите план: week, month, quarter, year');
                return;
            }

            const planKey = args[0].toLowerCase();
            const plan = this.config.rental.plans[planKey];
            
            if (!plan) {
                await this.bot.sendMessage(chatId, '❌ Неверный план. Доступные: week, month, quarter, year');
                return;
            }

            // Проверка существующей аренды
            const existingRental = this.bot.database.getRental(chatId);
            if (existingRental && this.bot.database.isRentalActive(chatId)) {
                await this.bot.sendMessage(chatId, '❌ Аренда уже активна. Используйте #продлитьаренду или #удалитьаренду');
                return;
            }

            // Создание аренды
            const rental = await this.bot.database.createRental(chatId, plan.name, plan.duration);
            
            let message = `✅ *Аренда создана!*\n\n`;
            message += `📋 Тариф: ${rental.plan}\n`;
            message += `⏰ Срок: ${plan.duration} часов\n`;
            message += `📅 До: ${moment(rental.endDate).format('DD.MM.YYYY HH:mm')}\n`;
            ...
            this.logger.error('Create rental error:', error);
            await this.bot.sendMessage(messageInfo.chatId, '❌ Ошибка создания аренды');
        }
    }

    /**
     * Команда: продлитьаренду - продление аренды (только владелец)  
     * Формат: #продлитьаренду <часы>
     */
    async extendRental(messageInfo, args) {
        try {
            const { chatId } = messageInfo;
            
            if (args.length === 0) {
                await this.bot.sendMessage(chatId, '❌ Укажите количество часов для продления');
                return;
            }

            const hours = parseInt(args[0]);
            if (isNaN(hours) || hours <= 0) {
                await this.bot.sendMessage(chatId, '❌ Некорректное количество часов');
                return;
            }

            const rental = this.bot.database.getRental(chatId);
            if (!rental) {
                await this.bot.sendMessage(chatId, '❌ Аренда не найдена. Создайте новую через #создатьаренду');
                return;
            }

            // Продление аренды
            const updatedRental = await this.bot.database.extendRental(chatId, hours);
            if (!updatedRental) {
                await this.bot.sendMessage(chatId, '❌ Ошибка продления аренды');
                return;
            }

            let message = `✅ *Аренда продлена!*\n\n`;
            message += `⏰ Добавлено: ${hours} часов\n`;
            message += `📅 Новая дата окончания: ${moment(updatedRental.endDate).format('DD.MM.YYYY HH:mm')}\n`;
            message += `📋 Осталось: ${moment(updatedRental.endDate).fromNow()}`;

            await this.bot.sendMessage(chatId, message);

        } catch (error) {
            this.logger.error('Extend rental error:', error);
            await this.bot.sendMessage(messageInfo.chatId, '❌ Ошибка продления аренды');
        }
    }
    ...
    async help(messageInfo, args) {
        try {
            ...
            if (isGroup && (isAdmin || isOwner)) {
                ...
                if (isOwner) {
                    message += `\n🔧 *Управление арендой (владелец):*\n`;
                    message += `• #создатьаренду <план> - создать аренду\n`;
                    message += `• #продлитьаренду <часы> - продлить аренду\n`;
                    message += `• #удалитьаренду - удалить аренду\n`;
                }
            } else {
